...
#personal coll params
    ...
    #Setup paths and constants depending on storage type
    if(-not (Set-StoragePermissions  -StorageType $StorageType -CentralStoragePath $CentralStoragePath -LocalStoragePath $LocalStoragePath `
                            -VmTemplateStoragePath $VirtualDesktopTemplateStoragePath -HostList $hostList))
    {
        return
    }
...
param(
    ...
    if(($vmObjs))
    {
        ...
        #for unmanaged pool, remove from collection
        else
        {
            Write-Verbose (Get-ResourceString RemovingVirtDesktopFromCollection)
...
param(
    ...
    {                
        ...
        $jobStatus=Get-VirtualDesktopCollectionJobStatus $CollectionName -ConnectionBroker $ConnectionBroker
        if(-not $jobStatus)
        {
            return
        }
...
param(
    ...
    else
    {
        ...
        $vmStatusMap = $jobReport.VmStatusMap;

        # create Job status object
        $jobStatus = New-Object  Microsoft.RemoteDesktopServices.Management.RDVirtualDesktopCollectionJobStatus `
            -ArgumentList $CollectionName, $status, $jobReport.JobStartTime, $jobReport.JobLastModifiedTime, $jobReport.TotalRequested, ($jobReport.PercentComplete.ToString() + '%'), $jobReport.TotalFailed, $jobReport.VmStatusMap.ToArray()
    }
...
param(
    ...
    foreach($VMHAHost in $HostList.Keys)
    {
        try
        {
            ...
            if($null -ne $HostList[$VMHAHost])
            {
                if($HostList[$VMHAHost].GetType().FullName -eq "System.Int32")
                {
                    if(($HostList[$VMHAHost] -ge 1) -and ($HostList[$VMHAHost] -le 5))
                    {
                        $VMHAParamKey.SetValue("Concurrency", $HostList[$VMHAHost])
                    }
                    else
                    {
                        Write-Error (Get-ResourceString ErrInvalidConcurrencyRange $HostList[$VMHAHost], $VMHAHost)
                    }
                }
                else
                {
                    Write-Error (Get-ResourceString ErrInvalidConcurrencyType $HostList[$VMHAHost].GetType().FullName, $VMHAHost)
                }
            }
        ...
        {
            Write-Error (Get-ResourceString ErrSettingConcurrencyFactor $VMHAHost, $_.Exception.Message)
        }
    }
}
...
param(
    ...
    {
        $errMsg = $_.Exception.InnerException.ErrorCode
    }
    catch 
    {
        $errMsg = $_.Exception.Message
    ...
    {
        Write-Error (Get-ResourceString FailedToGetMasterVmInfo $outParams.ReturnValue)
        return $false
    }
    ...
    foreach($VMHAHost in $HostList.Keys)
    {
        try
        {
            ...
            if($null -ne $HostList[$VMHAHost])
            {
                if($HostList[$VMHAHost].GetType().FullName -eq "System.Int32")
                {
                    $VMHAParamKey.SetValue("IdleVmCount", $HostList[$VMHAHost])
                }
                else
                {
                    Write-Error (Get-ResourceString ErrInvalidIdleVmCountType $HostList[$VMHAHost].GetType().FullName, $VMHAHost)
                }
            }
        ...
        {
            Write-Error (Get-ResourceString ErrSettingIdleVmCount $VMHAHost, $_.Exception.Message)
        }
    }
}
...
param(
    ...
    foreach($VMHAHost in $HostList.Keys)
    {
        try
        {
            $RemoteRegistry=[Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey([Microsoft.Win32.RegistryHive]::LocalMachine, $VMHAHost)
            $VMHAParamKey=$RemoteRegistry.CreateSubKey("SYSTEM\\CurrentControlSet\\Services\\VmHostAgent\\Parameters")
            New-Object Microsoft.RemoteDesktopServices.Management.RDVirtualDesktopIdleCount -ArgumentList $VMHAHost, $VMHAParamKey.GetValue("IdleVmCount")
        }
        catch
        {
            Write-Error (Get-ResourceString ErrReadingIdleVmCount $VMHAHost, $_.Exception.Message)
        }
    }
}
...
param(
    ...
    foreach($VMHAHost in $VirtualizationHosts)
    {
        try
        {
            $RemoteRegistry=[Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey([Microsoft.Win32.RegistryHive]::LocalMachine, $VMHAHost.Server)
            $VMHAParamKey=$RemoteRegistry.CreateSubKey("SYSTEM\\CurrentControlSet\\Services\\VmHostAgent\\Parameters" + "\\" + $CollectionName)
            $VMHAParamKey.SetValue($Name, $Value)            
        }
        catch
        {
            Write-Error (Get-ResourceString ErrSettingHostConfiguration $Name, $VMHAHost.Server, $_.Exception.Message)
            return $false
        }
    }    
    return $true
