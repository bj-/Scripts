...
#
# parameter binding for the whole script
#

################################################################
#.Synopsis
#  Exports Microsoft iSCSI Target 6.2/3.3/3.2 settings to a file specified.
#  Imports saved settings to Microsoft iSCSI Target 6.2.
#.Description
#  Performs export of Microsoft iSCSI Target 6.2/3.3/3.2 to a file.
#  Only the VirtualDisk/Target configuration
#  will be imported/exported. A complete list of VHD files
#  referenced by the configuration will be printed as part
#  of the export process.
#  Performs import of saved Microsoft iSCSI Target settings
#  to a Microsoft iSCSI Target 6.2.
#.Parameter Import
#  A switch telling to import settings from a file
#.Parameter Export
#  A switch telling to export settings to a file
#.Parameter Filename
#  The path to the file where the settings will be stored to or
#  imported from. On export, if the file does not exist, it will
#  be created.
# .Parameter ComputerName
#  The name of the computer (in NetBIOS, DNS, IPv4 or IPv6 fomat)
#  from which to export the settings or to which to apply
#  the settings. For a cluster, the name may be a node name,
#  the cluster name, or a client access point name.
#.Parameter Credential
#  Optional credentials to be used to connect to the remote
#  ComputerName. Actual password will be prompted for during execution.
#.Parameter Force
#  In the Export case, when Force is specified, the settings
#  file will be overwritten if it already exists.
#  In the Import case, when Force is specified, the settings
#  will be applied even if Target/VirtualDisks with same name exists.
#.Example
#  iSCSITargetSettings.ps1 -Export -Filename $ENV:TEMP\IscsiTargetSettings.xml
#    Exports iSCSITarget setting from the current computer to the specified file
#.Example
#  iSCSITargetSettings.ps1 -Export -Filename .\Settings-StorageServer-01.xml -Computer StorageServer-01.Contoso.com
#    Exports iSCSITarget setting from the specified computer to the specified file
#.Example
#  iSCSITargetSettings.ps1 -Import -Filename .\Settings-StorageServer-01.xml -Computer StorageServer-01.Contoso.com
#    Imports iSCSITarget settings to the specified computer reading them form the specified file
################################################################
param(
    [Parameter(ParameterSetName="ImportSet",Position=0,Mandatory=$true)]
    [System.Management.Automation.SwitchParameter]
    $Import,

    [Parameter(ParameterSetName="ExportSet",Position=0,Mandatory=$true)]
    [System.Management.Automation.SwitchParameter]
    $Export,
...
switch ($PsCmdlet.ParameterSetName)
{
    ...
    "ImportSet"
    {
        ...
        #
        if ( !($wtGeneral.Version -match '^v6.2') )
        {
            #
            # disallow importing on anything that is not iSCSI Target 6.2
            #
        ...
        #
        [bool]$isWin8NoImplicitImport = ($wtGeneral.Version -match '^v6.2');
        ...
        if ( $SettingsVersion -match '^v3.2' )
        {
            $isThor3 = $true;
        }
        elseif( $SettingsVersion -match '^v3.3' )
        {
            $isZamfir = $true;
        }
        ...
        if ($isCluster)
        {
            ...
            #
            # In win8, if not all the resource groups are online, we bail out
            #
            #
            ...
            #
            GetNodesClientAccessPoints $ComputerName ([ref]$ClusterNodes) ([ref]$ClientAccessPoints) ([ref]$ResoruceGroupsOfNetworkNames) $PSCredential;
        }
        ...
        #
        # These are the arrays for propert Fixed-before-VHD sorting
        #
        $ArrayFixedVHDs = @();
        ...
        {
            #
            # enumerate the VirtualDisk from the XML set
            # first the fixed VHD, then the Differencing
            #
            foreach($VirtualDisk in $xml.DocumentElement.iSCSIVirtualDiskRecords.ChildNodes)
            {
                ...
                {
                    #
                    # only fixed VHDs are supported in iSCSI Target 3.2
                    #
                    $ArrayFixedVHDs += $VirtualDisk;
                }
                elseif ( $isZamfir -or $isWin8 )
                {
                    ...
                    elseif ([System.Int32]$VirtualDisk.Type -eq $VhdTypeFixedVhd)
                    {
                        $ArrayFixedVHDs += $VirtualDisk;
                    }
                    elseif ([System.Int32]$VirtualDisk.Type -eq $VhdTypeDifferencingVhd)
                    {
        ...
        #
        $ArrayAllVHDs = $ArrayFixedVHDs + $ArrayDifferencingVHDs;
            ...
            foreach($VirtualDisk in $ArrayAllVHDs)
            {
                ...
                if ($RunImport)
                {
                    ...
                    try
                    {
                        ...
                        elseif ([string]::IsNullOrEmpty([string]$VirtualDisk.Type))
                        {
                            #
                            # cluster-resources WTDisks in the offline state in Zamfir
                            # do not have 'Type' exposed. They are assumed to be fixed,
                            # because if the file exists it has a locator to the parent path
                            #
                            
                            #
                            # If destination is Zamfir, import needs the size to be >= 8MB if file exists,
                            # if file does not exist, then it will create a new file
                            #
                            # If destination is Win8, import needs the size to be 0 if file exists,
                            # if file does not exist, then NewWTDisk will fail
                            #
                            $MinSize = 8;
                            if ( $isWin8NoImplicitImport )
                            {
                                $MinSize = 0;
                            }
                            $ReturnObj = $Wt_DiskClass.NewWTDisk($VirtualDisk.MigrationDevicePath,
                        ...
                        elseif ([System.Int32]$VirtualDisk.Type -eq $VhdTypeDifferencingVhd)
                        {
                            ...
                            else
                            {
                                $ReturnObj = $Wt_DiskClass.NewDiffWTDisk($VirtualDisk.MigrationDevicePath,
                                                                         $VirtualDisk.MigrationParentPath,
                                                                         $VirtualDisk.Description);
                            }
                        }
                        ...
                        if ( $null -ne $ReturnObj )
                        {
                            ...
                            if ( $null -ne $CreatedWtDisk )
                            {
                                ...
                                #
                                # only iSCSI Target 3.3 and above has Enabled property
                                #
                                if ($isZamfir -or $isWin8)
                                {
                                    #
                        ...
                        if ( $null -ne $ReturnObj )
                        {
                            ...
                            if ( $null -ne $CreatedWtHost )
                            {
                                ...
                                if (!$EnableState)
                                {
                                    ...
                                    {
                                        $FetchedWtHost.Enable = $EnableState;
                                    }
    ...
    "ExportSet"
    {
        ...
        [bool]$isZamfir = $false;
        [bool]$isWin8 = $false;
        if ( $wtGeneral.Version -match '^v3.2' )
        {
            $isThor3 = $true;
        }
        elseif( $wtGeneral.Version -match '^v3.3' )
        {
            $isZamfir = $true
        }
